...
The `anon` package provides three functions to help define succinct anonymous functions 
(also often referred to as *lambdas*).  
...
3. `formula_to_function()` is the simplest example of converting a formula
   ...
   takes a maximum of 3 arguments (`.x`, `.y`, `.z`)
...
The `anon` package aims to be comprehensive on what sort of formula construction
is allowed.  However it is not yet on CRAN (which can be inconvenient) and 
it is a dependency you may not want.
...
extracted from `anon` and used in your own code.  It has some severe restrictions

* Input can only be a 1-sided formula e.g. `~.x + 1`
* The created function takes 3 formals argument (`.x`, `.y`, `.z`) and they must be
...
and is easily copy/pasted into your code 
to maybe avoid incurring a dependency on `rlang` or `anon`
