...
`formula_to_1arg_function()` is a dependency-free way to create functions from 
formulas.  It has some severe restrictions

* Input can only be a 1-sided formula
* the created function only takes one formal argument called `.x`.  This is not configurable.

This code is easily copy/pasted into other packages without incurring a dependency on `rlang` or
`anon`
...
After excluding the error checking, the body of `formula_to_1arg_function()` is very simple:
